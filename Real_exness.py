...
account_number = 248191534  # Replace with your Deriv MT5 account number
password = input(f"Enter password for {account_number}: ")  # Replace with your actual password
server = "Exness-MT5Trial"  # Change to "Deriv-Real" if using a real account
...
FOREX_PAIRS = ["CHFJPYm","USDCHFm"]
XAUUSD = "XAUUSDm"
BTCUSD="BTCUSDm"
MAJORS = [ ]
US =["USTECm","US30m"]

TIMEFRAME_H1 = MAJORS+[XAUUSD,BTCUSD]+US
H1_B= [XAUUSD,BTCUSD]+US
H1_S=[]
H1_BS=MAJORS
H1_pen= MAJORS+[BTCUSD]
H1_pl= [XAUUSD]+US

TIMEFRAME_M15= FOREX_PAIRS+[XAUUSD]
M15_B= [XAUUSD]+FOREX_PAIRS
M15_S=[]
M15_BS= []
M15_pen= FOREX_PAIRS+[XAUUSD]
M15_pl= []
    ...
    for symbol,timeframe in current_active:
        try:
            ...
            apds = [
                ...
                mpf.make_addplot(trend_line, color='gray', width=1.5, linestyle='--', label='Trend Line'),
            ]
...
def signal_handler(sig, frame):
    ...
    send_telegram_message("Exness Bot stopped")
    ...
    for order in orders:
        ...
        if (order.type == mt5.ORDER_TYPE_BUY_LIMIT and price >= tp) or (order.type == mt5.ORDER_TYPE_SELL_LIMIT and price <= tp):
            ...
            if result and result.retcode == mt5.TRADE_RETCODE_DONE:
                send_telegram_message(f"Delete pending exness order for {symbol}.")
            ...
            for position in positions:
                ...
                (position.type == mt5.ORDER_TYPE_SELL and current_price <= Tp1):
                    if position.volume == get_min_lot_size(symbol):
                        ...
                        continue
                    if position.volume <= (get_min_lot_size(symbol) * 2 / 2):
                    ...
                    if position.sl == position.price_open:
                        ...
                        if close_result.retcode == mt5.TRADE_RETCODE_DONE:
                            send_telegram_message(f"TP1 hit. Apply breakeven and close half of exness positions for {symbol} trade.✅")
    ...
    if levels:
        for (symbol,timeframe) in list(levels.keys()):  # Use list to avoid runtime modification issues
            ...
            if (direction == "BUY" and current_price <= L["sl"]) or (direction == "SELL" and current_price >= L["sl"]):
                ...
                if (symbol, timeframe) in active_channels:
                    send_telegram_message(f"🚨 {symbol} hit Exness SL on {timeframe_to_str(timeframe)}")
                    del active_channels[(symbol,timeframe)]
                if (symbol, timeframe) in levels:
                    del levels[(symbol,timeframe)]
            ...
            if (direction == "BUY" and current_price >= L["tp2"]) or (direction == "SELL" and current_price <= L["tp2"]):
                ...
                if (symbol, timeframe) in active_channels:
                    plot_filename = os.path.join(PLOTS_FOLDER, f"{symbol}_{timeframe_to_str(timeframe)}_channel.png")
                    send_telegram_image(plot_filename, f"✅ {symbol} hit Exness TP2 on {timeframe_to_str(timeframe)}")
                    del active_channels[(symbol,timeframe)]
                if (symbol, timeframe) in levels:
                    del levels[(symbol,timeframe)]
